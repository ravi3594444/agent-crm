...
**No deploy script had ever run on this VM, and two separate things stopped them.** First, `plus-agent/.venv` existed with a `python` and *no pip and no packages* — `python3 -m venv` fails on this box until `sudo apt-get install -y python3.12-venv`. Second, and less obvious: `.env` has `ERPNEXT_URL=http://frontend:8080`, which only resolves **inside** the docker network. Run from the host, as `make seed` and `deploy/*.py` are documented to be, every script dies with `[Errno -3] Temporary failure in name resolution`. `app/__init__.py` calls `load_dotenv(..., override=False)`, so the fix is to pass the public URL on the command line, where it wins over `.env`:
`ERPNEXT_URL=https://agentcrm4.duckdns.org ERPNEXT_API_KEY=... ERPNEXT_API_SECRET=... .venv/bin/python deploy/cuentas_inventario.py`

**The `417` on Stock Reconciliation was NOT just the inventory accounts.** The accounts were genuinely missing and `deploy/cuentas_inventario.py --aplicar` set them — and the 417 stayed. The real message, which `app/erpnext.py` throws away (it keeps the status and drops the body), is:
`OpeningEntryAccountError: Difference Account must be a Asset/Liability type account, since this Stock Reconciliation is an Opening Entry`
ERPNext treats *any* Stock Reconciliation as an opening entry while **not one Stock Ledger Entry exists in the whole system**, and then refuses a P&L difference account — which is exactly what `Ajuste de Existencia` is, and what the company default correctly points at for everyday counts. Chicken-and-egg: no stock can be counted because no stock was ever counted. Broken on 16/09 by posting ONE opening entry by hand, submitted (a draft creates no ledger entry, and the ledger entry is the whole point):
`purpose="Opening Stock"`, `expense_account="Apertura Temporaria - LP"` (an Equity account created for this; the Argentine chart of accounts has no `Temporary Opening`), and a real `valuation_rate`. Every count after that works with the company default, from WhatsApp, with no code change. Two corollaries: `deploy/cuentas_inventario.py --probar` can never pass on a system with zero ledger entries — its probe posts the exact document ERPNext refuses — and an item that has never had stock still needs a `valuation_rate`, which `contar_stock` does not send.
...
4. ~~**Opening stock not loaded** — `417` on Stock Reconciliation; needs the company's inventory accounts set.~~ **Diagnosis was incomplete and the blocker is gone.** The inventory accounts *were* missing and are now set (`Stock In Hand - LP`, created under `1.1.6.00.00 - INVENTARIOS`), but that alone left the 417 in place: see the opening-entry gotcha above. `MAT-RECO-2026-00001` posted 5 kg of QUE-CRE at 4.500 on 16/09, and a count from WhatsApp (`MAT-RECO-2026-00002`, 5 → 12) then worked end to end, button and all. What remains is narrower: **the other twelve products still have no opening stock**, and a first-ever count of any of them needs a `valuation_rate` that `contar_stock` does not send.
