...
def _gerente(monkeypatch: pytest.MonkeyPatch) -> dict:
    """contar_stock authenticates the manager (Stage 2c) and offers a one-tap
    confirmation button; both are faked so only the ERPNext payload is tested."""
    monkeypatch.setattr(
        captura, "require_management", lambda config: SimpleNamespace(actor_phone="5493511111111")
    )
    monkeypatch.setattr(captura.notificar, "pedir_confirmacion_conteo", Mock(return_value=True))
    return {"configurable": {"thread_id": "ger:t", "actor_scope": "management", "actor_phone": "5493511111111"}}
...
def test_stock_count_without_change_does_not_create_a_document(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = captura.contar_stock.invoke({"item_code": "QUE-CRE", "cantidad_real": 12}, config=config)
