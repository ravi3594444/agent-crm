...
| 4 | marcador durable en un comentario de ERPNext: escribir, leer, y el techo de cuántos | ~~11~~ **12** marcadores (10 entre corchetes + 2 en prosa: el censo estaba corrido en uno), 0 lector compartido, `MAX_MARCAS` dos veces con valores distintos (5 y 20) | ~~**0**~~ **1–7** | — | 3 | **hecho**: `app/marcas.py` con registro (#24) |
...
**4 — Los marcadores durables. HECHO (#24).** `app/marcas.py` tiene el registro y el lector único, y `tests/test_marcas.py` fija los textos contra una tabla escrita a mano: el radio medido pasó de 0 a entre 1 y 7 según qué se mute. Dos correcciones que salieron de hacerlo: los marcadores son **doce**, no once —diez entre corchetes más los dos en prosa; el censo de abajo enumera diez y los llama once—, y 5 contra 20 **no era el mismo número con dos valores**: `sombra` pide `creation desc` y se queda con la más nueva, `confirmacion` pide `creation asc` y se queda con la más vieja, así que cada uno pide la página en la dirección de su extremo y truncar no puede dar una respuesta equivocada, sólo ninguna. El techo ahora se declara en la fila del marcador junto con su orden y su motivo, que es la única forma en que quiere decir algo. `confirmacion.py` quedó afuera de la consolidación por el brief, con su fila declarada y su `MAX_MARCAS` intacto. Lo que decía este párrafo antes, y sigue siendo el motivo: hay once —`[limite]`, `[entrega]`, `[idioma]`, `[accion]`, `[solicitud]`, `[sombra]`, `[confirmado-por-agente]`, `[remito-preparado-por-agente]`, `[pendiente-aviso]`, `[pendiente-cerrado]`, más dos en prosa (`Requiere revisión humana:`, `Rechazado manualmente por`)— y **ningún lector compartido**: `pendientes._tiene_marca`, `limites._consultar_marca`, `autonomia._comentarios` y `sombra`/`confirmacion` cada uno con su consulta y su parseo. El techo de cuántos leer está escrito dos veces con valores distintos (`sombra.MAX_MARCAS = 5`, `confirmacion.MAX_MARCAS = 20`) y nada dice por qué difieren. Lo que lo pone en la lista es el **radio 0**: cambié el texto de un marcador y no se rompió un solo test, así que el nombre del marcador —que es el formato durable con el que el sistema se entiende consigo mismo entre reinicios— no está protegido por nada. Un `marcas.py` con un registro `nombre → (texto, techo, parser)` y un lector único.
