...
si alguien escribía TELEFONOS_EQUIPO con espacios, con 0 o con 15, el dueño
no matcheaba `5493511234567` de Meta y quedaba ruteado como cliente, sin
acceso a sus propias herramientas.
"""
...
def _cargar_equipo() -> list[str]:
    crudos = [
        t.strip() for t in os.getenv("TELEFONOS_EQUIPO", "").split(",") if t.strip()
    ]
    normalizados: list[str] = []
    for t in crudos:
        n = telefono.normalizar(t)
        if not n:
            print(f"[router] TELEFONOS_EQUIPO: no pude interpretar {t!r}, lo ignoro")
            continue
        if n not in normalizados:
            normalizados.append(n)
    if not normalizados:
        print(
            "[router] TELEFONOS_EQUIPO está vacío: nadie puede aprobar pedidos ni "
            "usar el agente de gestión, y no se envían notificaciones."
        )
    return normalizados
